...
def create_video_from_image_and_audio(image_path: str, audio_path: str, output_video_path: str, fps: int = 24) -> str:
    ...
    This tool incorporates best practices for creating videos from static images, ensuring the output
    is highly compatible with web players and devices. It handles images with odd-numbered dimensions,
    standardizes audio, and optimizes the output file for web streaming.

    Args:
        image_path: Path to the input static image file.
        audio_path: Path to the input audio file.
        output_video_path: Path to save the resulting video file.
    ...
    try:
        ...
        # Get audio duration to explicitly set the video length.
        ...
        # --- Build Robust Video Filter Chain ---
        video_stream = image_input.video
        
        # 1. Pad to even dimensions (critical for yuv420p with libx264)
        video_stream = video_stream.filter('pad', width='ceil(iw/2)*2', height='ceil(ih/2)*2')

        # 2. Set pixel format for compatibility
        video_stream = video_stream.filter('format', pix_fmts='yuv420p')

        # --- Combine streams and set output options ---
        (
            ffmpeg
            .output(
                video_stream, 
        ...
        error_message = e.stderr.decode('utf8') if e.stderr else str(e)
        return f"Error creating video from image and audio: {error_message}"
    except (KeyError, ValueError) as e:
        return f"Error parsing duration from audio file metadata: {str(e)}"
    except Exception as e:
        return f"An unexpected error occurred: {str(e)}"
...
def add_b_roll(main_video_path: str, broll_clips: list[dict], output_video_path: str) -> str:
    ...
    """
    main_video_path = resolve_path(main_video_path)
    output_video_path = resolve_path(output_video_path)
    for clip in broll_clips:
        if 'clip_path' in clip:
            clip['clip_path'] = resolve_path(clip['clip_path'])
